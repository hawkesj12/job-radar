...
2. **Scores every role on one comparable scale** — a transparent, weighted keyword model you fully control in the config. Two properties do the work. **Each keyword counts once, however many times it appears**, so a posting can't buy rank by repeating "AI" forty times. And the **title and the description are scored as separate streams**, each measured against its own normal length (BM25F), so a feed that ships no description is treated as a _short_ posting rather than a _bad_ one — before this, the same role scored 30 via a feed with a description and 18 via one without, and the second copy was dropped. The weights are yours and hand-set; there's no IDF, so a keyword's value is what you say it is, not how rare it happens to be. Tuned for _recall_ by design (catch everything that might fit); the optional LLM re-rank below is the _precision_ layer.
