...
| **`remote_region`**                                     | **where a remote worker may sit** — an ISO alpha-2 country code (`BR`), an **ISO 3166-2** US state (`US-CA`, never a bare `CA`, because seven codes are both a US state and a country), a multi-country region (`EMEA`, `NORTH AMERICA`), or `ANY` when the posting really says anywhere. Parsed from the location for every source, not just the few that state it, and falling back to the row's own `country`. **`None` means UNSTATED, which is not `ANY`**: a bare `Remote` usually means "within whatever country the employer can pay from", so it is left unknown rather than defaulted to unbounded. Filter on it with `remote_regions` — the arrangement and the boundary are two different facts, and `Remote - Brazil` and `Remote` are the same arrangement with different boundaries.                                                                                |
...
title said "Remote" used to come out with `remote_type`, `remote_basis` and `remote_region`
all `None` — indistinguishable from a row nobody classified. An unknown arrangement stays
