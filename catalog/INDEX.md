...
dated; `rights: unknown` survives on nodesk and teamtailor only, where the terms could not be
...
13 `unclear`, 1 `trial_only`, 2 `unknown`. Nothing is `allowed`. Two of the six prohibited —
`techtree` and `usajobs` — ship in the package today, and `weworkremotely` was the headline new
adapter in the 0.7.0 plan before its API terms were read.
...
| braintrust                                         | 🔵 wired         | **yes**             | **silently ignores** | unknown  | unknown | unclear        | `app.usebraintrust.com/api/jobs/?limit=20`                           | 2026-08-03 | emits a `level` currently misrouted into `department`                                                                                                                                                                                                                                                                                                                           |
